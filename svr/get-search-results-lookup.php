...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) { // output data of each row
        ...
        $inner_link=$inner_link.'<a href="input.php?search_id='.$row["id"].'&search_name='.$row["first_name"].'">'.$row["first_name"].' '.$row["sur_name"]."</a> ";
    }
...
if ($results->num_rows > 0) {
    ...
    while($row = $results->fetch_assoc()) {
        ...
        $first_gen_family_table=$first_gen_family_table.  ' <td><a href="input.php?search_id='.$row["id"].'&search_name='.$row["first_name"].'">Use This</a></td>';
       $first_gen_family_table=$first_gen_family_table.  '</tr>';
      }
      //close table
      $first_gen_family_table=$first_gen_family_table.  ' </tr>';
